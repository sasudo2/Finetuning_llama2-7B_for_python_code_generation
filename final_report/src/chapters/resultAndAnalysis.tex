...
\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{src/images/figures/loss.png}
...
\subsubsection{Example Success Case}

\textbf{Problem:}

Create a function \texttt{from typing import List\n\n\ndef filter\_by\_substring(strings: List[str], substring: str) -> List[str]:}
that filter an input list of strings only for ones that contain given substring.
...
\subsubsection{Insights from Success Case}

The analysis of this example demonstrates that the fine-tuned model exhibits:

\begin{itemize}
\item \textbf{Correct syntactic and structural Python code generation: }the function filter\_by\_substring is valid Python and can be executed without errors.
\item \textbf{Accurate logical implementation: }the function filter\_by\_substring is valid Python and can be executed without errors.
\item \textbf{Concise and readable code: }the list comprehension is an efficient and Pythonic way to express the desired functionality.
\item \textbf{Robustness for typical inputs: }it handles empty strings, empty lists, and strings with special characters appropriately.
\end{itemize}

These observations suggest that the model:

\begin{itemize}
\item \textbf{Effectively generalizes from training examples: }the model can translate a natural language requirement into working Python code.
\item \textbf{Produces memory-efficient implementations: }using list comprehensions avoids unnecessary loops or temporary variables.
\item \textbf{Requires minimal human intervention: }the generated code usually needs little to no modification for standard use cases.
\end{itemize}

Optional improvements for future fine-tuning:

\begin{itemize}
\item Incorporating more edge cases, such as very large input lists or Unicode characters, to ensure performance and correctness in all scenarios.
\item Adding optional parameters (e.g., case-insensitive filtering) to enhance utility without compromising simplicity.
\end{itemize}
...
\textbf{Problem:}
Create a function \texttt{def separate\_paren\_groups(paren\_string: str) -> List[str]:} 
that takes a single string as input. This string contains multiple groups of nested 
parentheses. Your goal is to identify each independent, balanced group, separate them 
into individual strings, and return them as a list.
...
The final model was converted to GGUF format and deployed via Ollama. Performance on a local NVIDIA RTX 4050 showed an average inference speed of \textbf{15.67 tokens per second}.
...
Time to First Token & 0.52s \\
Average Tokens Per Second & 15.67 t/s \\
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\textwidth]{src/images/figures/system_usage.png}
    \caption{Resource Usage By Ollama During Inference}
    \label{fig:Resource usage by ollama during inference}
\end{figure}
