...
Another limitation is that the observed performance gain was limited. Although the fine-tuned model showed improvements over baseline behavior in several metrics, the margin was modest and not consistently large across all evaluation settings.

The current work is also restricted to the Python domain. As a result, the findings cannot be directly generalized to other programming languages without additional fine-tuning and validation.
...
No real-world deployment tests were conducted in this study. Therefore, system behavior under production constraints such as latency, scalability, robustness, and user interaction remains unverified.

In addition, Reinforcement Learning from Human Feedback (RLHF) was not applied. The model was optimized through supervised fine-tuning only, which may limit alignment quality and response refinement compared to RLHF-based pipelines.
